...
import { nethook ,PacketId, NetworkIdentifier } from 'bdsx';
import { ModalFormRequestPacket, SetTitlePacket } from "bdsx/bds/packets"
...
export function sendModalForm(networkIdentifier: NetworkIdentifier, form: object, callback: (data: any, networkIdentifier: any) => void = function() {}) {
    ...
    let packet = ModalFormRequestPacket.create();
    packet.id = formId;
...
nethook.raw(PacketId.ModalFormResponse).on((ptr, _size, networkIdentifier) => {
    ptr.move(1);
